...
export class OpenRouterService {
  ...
  async #countTokens(model: string, ...texts: (string | undefined)[]): Promise<number> {
    try {
      ...
      return Math.ceil(texts.reduce((sum, text) =>
        sum + (text?.length ?? 0), 0) / 4);
    }
  }
  ...
  async #validateResponse(data: unknown, schema?: JsonSchemaFormat): Promise<void> {
    ...
    try {
      ...
      const validate = this.#ajv.getSchema(schema.json_schema.name) ??
        this.#ajv.compile(schema.json_schema.schema);
      ...
      if (!valid) {
        ...
        throw new ValidationError(
          `Response validation failed: ${errors?.[0]?.message ?? 'Unknown error'}`,
          { data, errors }
        );
      }
    } catch (error) {
